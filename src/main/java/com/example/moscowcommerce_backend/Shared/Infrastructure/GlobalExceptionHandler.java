...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<Result<Map<String, String>>> handleValidationExceptions(
    ...
    return new ResponseEntity<>(
        Result.failure("Surgió un error con los datos proporcionados", errors),
        HttpStatus.BAD_REQUEST);
  }

  // Manejo de excepciones de autorización no válida
  @ExceptionHandler(UnauthorizedException.class)
  public ResponseEntity<Result<Map<String, String>>> handleUnauthorizedException(
      UnauthorizedException ex) {
    Map<String, String> errorResponse = new HashMap<>();
    errorResponse.put("error", ex.getMessage()); // Puedes personalizar el mensaje
    return new ResponseEntity<>(
        Result.failure("Error al autenticar ", errorResponse), HttpStatus.UNAUTHORIZED);
  }

  @ExceptionHandler(InvalidTokenException.class)
  public ResponseEntity<Result<Map<String, String>>> handleInvalidTokenException(
      InvalidTokenException ex) {
    Map<String, String> errorResponse = new HashMap<>();
    errorResponse.put("error", ex.getMessage()); // Puedes personalizar el mensaje
    return new ResponseEntity<>(
        Result.failure("Token inválido", errorResponse), HttpStatus.UNAUTHORIZED);
  }
  ...
  public ResponseEntity<Result<Map<String, String>>> handleGlobalException(Exception ex) {
    ...
    return new ResponseEntity<>(
        Result.failure("Error inesperado", errorResponse), HttpStatus.INTERNAL_SERVER_ERROR);
  }
}
