...
public class JPAUserRepository implements IUserRepository {
  ...
  public Optional<User> findByEmail(String email) {
    return userRepositoryJPA.findByEmail(email).map(userEntity -> toDomain(userEntity));
  }

  @Override
  public Optional<User> findById(Integer id) {
    return userRepositoryJPA.findById(id).map(userEntity -> toDomain(userEntity));
  }

  @Override
  public List<User> findAll() {
    return userRepositoryJPA.findAll().stream().map(userEntity -> toDomain(userEntity)).toList();
  }
