...
public class UpdateProductService implements IUpdateProductService {
  ...
  public ProductEntity updateProduct(Product updateProduct) {
    ...
    Optional<ProductEntity> existingProductOpt =
        this.productRepository.findById(updateProduct.getId());
    ...
    String name =
        updateProduct.getName() != null ? updateProduct.getName() : existingProduct.getName();
    String description =
        updateProduct.getDescription() != null
            ? updateProduct.getDescription()
            : existingProduct.getDescription();
    Integer stock =
        updateProduct.getStock() > 0 ? updateProduct.getStock() : existingProduct.getStock();
    Double price =
        updateProduct.getPrice() > 0 ? updateProduct.getPrice() : existingProduct.getPrice();
    Category category =
        updateProduct.getCategory() != null
            ? updateProduct.getCategory()
            : CategoryMapper.toDomainFromEntity(existingProduct.getCategory());
    ...
    if (updateProduct.getPhotos() != null && !updateProduct.getPhotos().isEmpty()) {
      ...
      List<ProductPhotoEntity> updatedPhotos =
          updateProduct.getPhotos().stream()
