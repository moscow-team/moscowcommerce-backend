...
public class CreateProductService implements ICreateProductService {
    ...
    public void create(Product product) {
        ...
        // Lo hacemos verificando si existe un producto con este id, y si existe devolvemos la excepcion.
        Optional<ProductEntity> productExist = this.productRepository.findById(product.getId());
        if(productExist.isPresent()){
            throw new ProductAlreadyExistsException();
        }
        // Primero debemos crear el producto sin las fotos ya que no podriamos referenciar las fotos a un producto no creado
