...
public final class CreateOrderUseCase implements ICreateOrderUseCase {
  ...
  private List<Product> validateAndRetrieveProducts(List<ProductOrderDTO> productOrders) {
    return productOrders.stream().map(this::validateAndReduceStock).collect(Collectors.toList());
  }

  private Order saveOrderAndUpdateProducts(
      Order order, List<Product> products, List<ProductOrderDTO> productOrdersDTO) {
    ...
    productOrdersDTO.forEach(
        productOrder -> {
          for (int i = 0; i < productOrder.getQuantity(); i++) {
            productsToSaveOrder.add(
                products.stream()
  ...
  private Product validateAndReduceStock(ProductOrderDTO productOrder) {
    Product product =
        productRepository
            .findById(productOrder.getProductId())
    ...
    if (product.getStock() < productOrder.getQuantity()) {
      throw new InsufficientStockExeption(
          "El producto con id "
              + product.getId()
              + " no tiene suficiente stock. Stock disponible: "
              + product.getStock());
    }
