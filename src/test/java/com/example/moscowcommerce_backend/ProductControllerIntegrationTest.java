...
public class ProductControllerIntegrationTest {
  ...
  void createProductSuccessfully() throws Exception {
    ...
    productDTO.setPrice(2000.0878969696785);
  ...
  void deleteProductSuccessfully() throws Exception {
    mockMvc
        .perform(delete("/products/" + createdProductId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void deleteNonExistentProduct() throws Exception {
    mockMvc
        .perform(delete("/products/999").contentType(MediaType.APPLICATION_JSON))
  ...
  void unarchiveProductSuccessfully() throws Exception {
    ...
    mockMvc.perform(
        delete("/products/" + createdProductId).contentType(MediaType.APPLICATION_JSON));

    // Desarchivar el producto
    mockMvc
        .perform(patch("/products/" + createdProductId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void unarchiveNonExistentProduct() throws Exception {
    mockMvc
        .perform(patch("/products/999").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false));
  }
  ...
  void updateProductSuccessfully() throws Exception {
    ...
    updateProductDTO.setPrice(333.99);
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.put(
                    "/products/" + createdProductId) // Usar PUT para la actualización
                ...
                .contentType(
                    MediaType.MULTIPART_FORM_DATA) // Definir que el contenido es de tipo multipart
            )
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void updateNonExistentProduct() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.put("/products/999") // Usar PUT para la actualización
                .param("name", updateProductDTO.getName())
                .param("description", updateProductDTO.getDescription())
                .param("price", String.valueOf(updateProductDTO.getPrice()))
                .param("stock", String.valueOf(updateProductDTO.getStock()))
