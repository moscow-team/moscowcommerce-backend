...
public class CategoryControllerIntegrationTest {
    ...
    void createCategoryWithExistingName() throws Exception {
        ...
        mockMvc.perform(post("/category")
                ...
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Ya existe una categoría con el nombre " + MATERA_NAME));
    }
    ...
    void unarchivedNonExistentCategory() throws Exception {
        ...
        mockMvc.perform(patch("/category/999")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
    ...
    void getCategoriesByCriteriaSuccessfully() throws Exception {
        mockMvc.perform(get("/category/filters?name=Test")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void getCategoriesByInvalidCriteria() throws Exception {
        mockMvc.perform(get("/category/filters?invalidParam=value")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false));
    }
